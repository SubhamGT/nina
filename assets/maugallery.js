...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(lightboxId){let activeImageSrc=$(`#${lightboxId} .lightboxImage`).attr("src");let imagesCollection=[];let activeTag=$(".tags-bar span.active-tag").data("images-toggle");if(activeTag==="all"){$(".gallery-item").each(function(){imagesCollection.push($(this).attr("src"))})}else{$(".gallery-item").each(function(){if($(this).data("gallery-tag")===activeTag){imagesCollection.push($(this).attr("src"))}})}
let index=imagesCollection.indexOf(activeImageSrc);let prevIndex=(index-1+imagesCollection.length)%imagesCollection.length;$(`#${lightboxId} .lightboxImage`).attr("src",imagesCollection[prevIndex])},nextImage(lightboxId){let activeImageSrc=$(`#${lightboxId} .lightboxImage`).attr("src");let imagesCollection=[];let activeTag=$(".tags-bar span.active-tag").data("images-toggle");if(activeTag==="all"){$(".gallery-item").each(function(){imagesCollection.push($(this).attr("src"))})}else{$(".gallery-item").each(function(){if($(this).data("gallery-tag")===activeTag){imagesCollection.push($(this).attr("src"))}})}
let index=imagesCollection.indexOf(activeImageSrc);let nextIndex=(index+1)%imagesCollection.length;$(`#${lightboxId} .lightboxImage`).attr("src",imagesCollection[nextIndex])},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
...
$(".tags-bar .nav-link").removeClass("active active-tag");$(this).addClass("active active-tag");var tag=$(this).data("images-toggle");$(".gallery-item").each(function(){$(this).parents(".item-column").hide();if(tag==="all"){$(this).parents(".item-column").show(300)}else if($(this).data("gallery-tag")===tag){$(this).parents(".item-column").show(300)}})}}})(jQuery)